...
def pig_pass(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4b
    "*** YOUR CODE HERE ***"
    d_score = opponent_score - player_score
    if d_score >= 1 and d_score <= 2:
        return True
    else:
        return False
    # END PROBLEM 4b
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        ...
        if who == 0:
            if new_running_high < score0 - last_score:
                new_running_high = score0 - last_score
                print(new_running_high, 'point(s)! The most yet for Player', who)
            return announce_highest(0, score0, new_running_high)
        else:
            if new_running_high < score1 - last_score:
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    while i <= 10:
        averaged = averaged_dice(i, dice)
        if averaged > maxv:
            maxv, maxi = averaged, i
        i = i + 1
    return maxi
    # END PROBLEM 9
