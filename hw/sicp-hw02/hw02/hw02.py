...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(k, x, direction):
        if n == k:
            return x
        else:
            if k < n:
                if k % 8 == 0 or num_eights(k):
                    return helper(k + 1, x - direction, direction * -1)
                else:
                    return helper(k + 1, x + direction, direction)
    return helper(1, 1, 1)
...
def missing_digits(n):
    ...
    def helper(n, x):
        if n < 10:
            ...
            if n % 10 == (n // 10) % 10:
                return helper(n // 10, x)
            else:
                return helper(n // 10, x + (n % 10 - (n // 10) % 10 - 1)) 
